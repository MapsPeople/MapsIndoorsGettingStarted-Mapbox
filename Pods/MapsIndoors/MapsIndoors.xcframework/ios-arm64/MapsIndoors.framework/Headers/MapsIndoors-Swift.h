...
/// Make no camera changes.
  MPCameraViewFitModeNone = 3,
};
...
@property (nonatomic, weak) id <MPFloorSelectorDelegate> _Nullable delegate;
...
- (id <MPDataSetCache> _Nullable)addDataSet:(NSString * _Nonnull)dataSetId;
/// Add data set to the list of managed data sets.
/// \param dataSetId Data set id of the data set that should be added.
///
...
/// Additional Location stops between <code>origin</code>/<code>originPoint</code> and <code>destination</code>/<code>destinationPoint</code>.
/// The resulting route will visit the stops in the specified order unless <code>optimizeRoute</code> is <code>true</code>.
/// Using this takes precedence over <code>stopsPoints</code>.
@property (nonatomic, copy) NSArray<id <MPLocation>> * _Nullable stops;
/// Additional Location stops between <code>origin</code>/<code>originPoint</code> and <code>destination</code>/<code>destinationPoint</code>.
/// The resulting route will visit the stops in the specified order unless <code>optimizeRoute</code> is <code>true</code>.
@property (nonatomic, copy) NSArray<MPPoint *> * _Nullable stopsPoints;
/// Set this to <code>true</code> to reorder the provided <code>stops</code> to get the shortest route between the stops.
@property (nonatomic) BOOL optimizeRoute;
...
@class UIColor;
@protocol MPRoute;
@protocol MPRouteStopIconProvider;
...
/// Enable/disable the route end/start label buttons from showing on the route. Default is <code>true</code>.
...
/// The background color of the rendered route
...
/// The value of <code>animationDuration</code> is updated to <code>duration</code>.
...
- (void)update;
/// Render the route on the map. If <code>animationDuration</code> is different from <code>0</code> the route will animate.
- (void)render;
/// Render the route on the map.
/// \param stopIcons A key/value mapping between the stop index and the icon provider which will be used to derive the stop icon. Stop indices not included in this dictionary will recieve the icon provided by the default set on <code>defaultRouteStopIcon</code>
///
- (void)renderWithStopIcons:(NSDictionary<NSNumber *, id <MPRouteStopIconProvider>> * _Nonnull)stopIcons;
/// The duration of camera animations in seconds. Set to <code>0</code> to disable animation. Default value is 5 seconds.
@property (nonatomic) NSTimeInterval animationDuration;
/// The default route stop icon provider - you can set your own implementation of <code>MPRouteStopIconProvider</code> here.
@property (nonatomic, strong) id <MPRouteStopIconProvider> _Nullable defaultRouteStopIcon;
@end


/// Delegate for reacting to directions renderer state changes.
SWIFT_PROTOCOL("_TtP11MapsIndoors28MPDirectionsRendererDelegate_")
@protocol MPDirectionsRendererDelegate
@optional
/// Invoked when the directions renderer decides to change the floor index. E.g. by selecting a leg which is on another floor.
- (void)onDirectionsRendererChangedFloorWithFloorIndex:(NSInteger)floorIndex;
/// Invoked when the directions renderer’s selected leg index changes.
- (void)onLegSelectedWithLegIndex:(NSInteger)legIndex;
@end
...
/// Scale of the icon image, a multiplier for the <code>iconSize</code>.
...
/// The text font size for the label.
@property (nonatomic) NSInteger labelStyleTextSize;
/// The text color for the label.
@property (nonatomic, strong) UIColor * _Nullable labelStyleTextColor;
/// The text opacity for the label.
@property (nonatomic) double labelStyleTextOpacity;
/// The text halo color for the label.
@property (nonatomic, strong) UIColor * _Nullable labelStyleHaloColor;
/// The text halo width for the label.
@property (nonatomic) NSInteger labelStyleHaloWidth;
/// The text halo blur for the label.
@property (nonatomic) NSInteger labelStyleHaloBlur;
/// The label bearing (only applies if <code>MPLabelType</code> is flat)  - only affects Mapbox users
@property (nonatomic) double labelStyleBearing;
/// The image used as a stretchable background for graphic labels - only affects Mapbox users
@property (nonatomic, copy) NSString * _Nonnull labelStyleGraphicBackgroundImage;
/// The stretch points for the grahic label background image, along the X-axis. These points represent areas in the image where it can be safely stretched - only affects Mapbox users
@property (nonatomic, copy) NSArray<NSArray<NSNumber *> *> * _Nonnull labelStyleGraphicStretchX;
/// The stretch points for the grahic label background image, along the Y-axis. These points represent areas in the image where it can be safely stretched - only affects Mapbox users
@property (nonatomic, copy) NSArray<NSArray<NSNumber *> *> * _Nonnull labelStyleGraphicStretchY;
/// Defines the text content within the graphic label background image  - only affects Mapbox users
@property (nonatomic, copy) NSArray<NSNumber *> * _Nonnull labelStyleGraphicContent;
...
/// The lightness factor is a modifier applied to the polygon fill color, and is interpreted as a percentage increase towards light or dark (-1 to 1, meaning -100% to 100%) of the lightness component in HSL color space.
@property (nonatomic) double polygonLightnessFactor;
...
/// Whether or not to show the location extrusion on the map.
@property (nonatomic) BOOL extrusionVisible;
/// The lightness factor is a modifier applied to the extrusion color, and is interpreted as a percentage increase towards light or dark (-1 to 1, meaning -100% to 100%) of the lightness component in HSL color space.
@property (nonatomic) double extrusionLightnessFactor;
...
/// Whether or not to show the location walls on the map.
@property (nonatomic) BOOL wallsVisible;
/// The lightness factor is a modifier applied to the wall color, and is interpreted as a percentage increase towards light or dark (-1 to 1, meaning -100% to 100%) of the lightness component in HSL color space.
@property (nonatomic) double wallsLightnessFactor;
...
/// The zoom level from where the icon badge should start showing on the map.
@property (nonatomic) double badgeZoomFrom;
/// The zoom level from where the icon badge should stop showing on the map.
@property (nonatomic) double badgeZoomTo;
/// The icon badge’s visibility.
@property (nonatomic) BOOL badgeVisible;
/// The icon badge’s radius.
@property (nonatomic) NSInteger badgeRadius;
/// The icon badge’s fill color.
@property (nonatomic, strong) UIColor * _Nullable badgeFillColor;
/// The icon badge’s stroke color.
@property (nonatomic, strong) UIColor * _Nullable badgeStrokeColor;
/// The icon badge’s stroke width.
@property (nonatomic) double badgeStrokeWidth;
/// The icon badge’s scale, which is a multiplicative modifier to the badge’s radius.
...
- (MPDisplayRule * _Nonnull)copyWithNewParent:(MPDisplayRule * _Nonnull)newParent SWIFT_WARN_UNUSED_RESULT;
...
/// The display rule used for drawing locations in a <code>MPLocationState/selected</code> state. Values in this display rule takes precedens over a location’s own display rule when a location is shown on the map
  MPDisplayRuleTypeSelection = 0,
/// The display rule used for drawing locations in a <code>MPLocationState/highlighted</code> state. Values in this display rule takes precedens over a location’s own display rule when a location is shown on the map.
  MPDisplayRuleTypeHighlight = 1,
/// The display rule used for drawing outline of the current building, only parts of this display rule is respected.
  MPDisplayRuleTypeBuildingOutline = 2,
  MPDisplayRuleTypeSelectionHighlight = 3,
/// The display rule used for drawing the blue dot, only parts of this display rule is respected.
  MPDisplayRuleTypeBlueDot = 4,
...
typedef SWIFT_ENUM(NSInteger, MPError, open) {
  ...
  MPErrorHttpError = 6,
  MPErrorNetworkUnreachable = 7,
  MPErrorCategoriesNotFound = 8,
  MPErrorBookingParameterInvalid = 9,
  MPErrorRouteStopsNotAllowedWithMultipleVenues = 10,
  MPErrorRouteStopsNotAllowedForExternalRoutes = 11,
  MPErrorUnknownError = 12,
};
static NSString * _Nonnull const MPErrorDomain = @"MapsIndoors.MPError";

/// Defines types of MapsIndoors rendered features
typedef SWIFT_ENUM(NSInteger, MPFeatureType, open) {
  MPFeatureTypeModel2D = 0,
  MPFeatureTypeModel3D = 1,
  MPFeatureTypeWalls2D = 2,
  MPFeatureTypeWalls3D = 3,
  MPFeatureTypeExtrusion3D = 4,
  MPFeatureTypeExtrudedBuildings = 5,
};
...
/// Highlight behavior class that determines how the result of applying a highlight should be displayed on the map. Get the default behavior from <code>MPHighlightBehavior/default</code>. The default behavior is no camera movement and no infowindow shown.
SWIFT_CLASS("_TtC11MapsIndoors19MPHighlightBehavior")
@interface MPHighlightBehavior : NSObject <MPMapBehavior>
/// Default filter behavior
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong, getter=default) MPHighlightBehavior * _Nonnull default_;)
+ (MPHighlightBehavior * _Nonnull)default SWIFT_WARN_UNUSED_RESULT;
/// Whether <code>MPMapControl</code> should move the camera and fit the map view to the filtered locations. The default behavior is no camera movement (<code>false</code>).
@property (nonatomic) BOOL moveCamera;
/// Whether <code>MPMapControl</code> should show the info window if a filter only contains one Location. The default behavior is no info window is shown (<code>false</code>).
@property (nonatomic) BOOL showInfoWindow;
...
/// Initialise with a specific route element type.
- (nonnull instancetype)initWithType:(enum MPHighwayType)type OBJC_DESIGNATED_INITIALIZER;
...
/// \param highways Array of <code>MPHighway</code>s.
///
///
/// returns:
/// Array of <code>MPHighway</code> names.
...
enum MPLocationState : NSInteger;
@protocol MPType;
...
/// Settings applying this location
@property (nonatomic, strong) MPLocationSettings * _Nullable locationSettings;
/// Get the location’s state
@property (nonatomic) enum MPLocationState state;
/// Get the location’s type
@property (nonatomic, strong) id <MPType> _Nullable typeRef;
@end
...
SWIFT_CLASS("_TtC11MapsIndoors18MPLocationSettings")
@interface MPLocationSettings : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
/// <blockquote>
/// Warning: [INTERNAL - DO NOT USE]
///
/// </blockquote>
- (MPLocationSettings * _Nullable)initWithDictionary:(NSDictionary * _Null_unspecified)dict error:(NSError * _Nullable * _Nullable)error SWIFT_METHOD_FAMILY(none) SWIFT_WARN_UNUSED_RESULT;
@end
...
/// INTERNAL - DO NOT USE
typedef SWIFT_ENUM(NSInteger, MPLocationState, open) {
  MPLocationStateNone = 0,
  MPLocationStateSelected = 1,
  MPLocationStateHighlighted = 2,
};
...
enum MPSelectionMode : NSInteger;
...
@property (nonatomic) enum MPSelectionMode floorSelectionMode;
@property (nonatomic) enum MPSelectionMode buildingSelectionMode;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// A list of map features which won’t be rendered (Only applicable on Mapbox implementations). Default is empty.
/// Use <code>MPFeatureType</code> values.
@property (nonatomic, copy) NSArray<NSNumber *> * _Nonnull hiddenFeatures;
/// Select a Location.
...
/// Apply a highlight to the map. Only highlight the <code>MPLocations</code> captured by the <code>MPFilter</code>.
/// \param filter The <code>MPFilter</code> to apply for which Locations to highlight on the map.
///
/// \param behavior The <code>MPHighlightBehavior</code> to apply.
///
- (void)setHighlightWithFilter:(MPFilter * _Nonnull)filter behavior:(MPHighlightBehavior * _Nonnull)behavior;
/// Apply a highlight to the map. Only highlight the <code>MPLocation</code>s included in the list.
/// \param locations An array of <code>MPLocation</code> to highlight on the map.
///
/// \param behavior The <code>MPHighlightBehavior</code> to apply.
///
- (void)setHighlightWithLocations:(NSArray<id <MPLocation>> * _Nonnull)locations behavior:(MPHighlightBehavior * _Nonnull)behavior;
...
/// The order in which the stop indexes are shown.
/// E.g. not optimised: <code>[0,1,2,3]</code>, optimised <code>[3,1,2,0]</code>
@property (nonatomic, readonly, copy) NSArray<NSNumber *> * _Nullable orderedStopIndexes;
...
@property (nonatomic, copy) NSString * _Nullable floor_name;
/// Label for displaying contextual information about this coordinate.
@property (nonatomic, copy) NSString * _Nullable label;
/// Latitude angle.
@property (nonatomic, strong) NSNumber * _Nonnull lat;
/// Longitude angle.
@property (nonatomic, strong) NSNumber * _Nonnull lng;
/// Floor level index.
@property (nonatomic, strong) NSNumber * _Nonnull zLevel;
@end

enum MPRouteLegBreakReason : NSInteger;
...
@property (nonatomic, readonly) enum MPRouteLegBreakReason legStartReason;
@property (nonatomic, readonly) enum MPRouteLegBreakReason legEndReason;
...
@property (nonatomic, readonly, strong) NSNumber * _Nullable stopIndex;
@end

typedef SWIFT_ENUM(NSInteger, MPRouteLegBreakReason, open) {
  MPRouteLegBreakReasonContextChange = 0,
  MPRouteLegBreakReasonFloorChange = 1,
  MPRouteLegBreakReasonRouteEnd = 2,
  MPRouteLegBreakReasonRouteStart = 3,
  MPRouteLegBreakReasonStopReached = 4,
};
...
@property (nonatomic, readonly, strong) id <MPRouteCoordinate> _Nonnull start_location;
@property (nonatomic, readonly, copy) NSArray<id <MPRouteStep>> * _Nullable steps;
...
SWIFT_PROTOCOL("_TtP11MapsIndoors23MPRouteStopIconProvider_")
@protocol MPRouteStopIconProvider
...
typedef SWIFT_ENUM(NSInteger, MPSelectionMode, open) {
  MPSelectionModeManual = 0,
  MPSelectionModeAutomatic = 1,
};
...
@class MPStateDisplayRuleCollection;
...
@property (nonatomic, strong) MPLocationSettings * _Nonnull locationSettings;
@property (nonatomic, strong) MPStateDisplayRuleCollection * _Nullable stateDisplayRules;
/// Enables new POI selection styling (driven by a new display rule, configurable via CMS). Default is true.
@property (nonatomic) BOOL newSelection;
/// <blockquote>
/// Warning: [INTERNAL - DO NOT USE]
///
/// </blockquote>
...
/// A collection of state display rules - a special type of display rule which only apply to location’s in a given state e.g. <code>highlighted</code>
SWIFT_CLASS("_TtC11MapsIndoors28MPStateDisplayRuleCollection")
@interface MPStateDisplayRuleCollection : NSObject
/// <blockquote>
/// Warning: [INTERNAL - DO NOT USE]
///
/// </blockquote>
- (MPStateDisplayRuleCollection * _Nullable)initWithDictionary:(NSDictionary * _Null_unspecified)dict error:(NSError * _Nullable * _Nullable)error SWIFT_METHOD_FAMILY(none) SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// Settings applying to locations of this type (Note: only where a location itself doesn’t have a value for a setting)
@property (nonatomic, strong) MPLocationSettings * _Nullable locationSettings;
@end
...
/// Load a MapsIndoors solution using an API key, limited to a subset of venues.
- (void)loadWithApiKey:(NSString * _Nonnull)apiKey venueIds:(NSArray<NSString *> * _Nonnull)venueIds completionHandler:(void (^ _Nonnull)(NSError * _Nullable))completionHandler;
...
- (void)shutdown;
/// A list of venue IDs that will be loaded when the SDK is initialized or synchronized. If this list is nil all venues are loaded. If this list is empty, the solution will load with no venues.
@property (nonatomic, readonly, copy) NSArray<NSString *> * _Nullable venuesToSync;
/// Add venue IDs that will be loaded when the SDK is initialized or synchronized
- (void)addVenuesToSyncWithVenueIds:(NSArray<NSString *> * _Nonnull)venueIds completionHandler:(void (^ _Nonnull)(NSError * _Nullable))completionHandler;
/// Remove venue IDs that will be loaded when the SDK is initialized or synchronized
- (void)removeVenuesToSyncWithVenueIds:(NSArray<NSString *> * _Nonnull)venueIds completionHandler:(void (^ _Nonnull)(NSError * _Nullable))completionHandler;
...
/// Search for MPLocations using MPQuery and/or MPFilter. - without logging, for internal SDK usage
- (void)internalLocationsLookupWithQuery:(MPQuery * _Nullable)query filter:(MPFilter * _Nullable)filter completionHandler:(void (^ _Nonnull)(NSArray<id <MPLocation>> * _Nonnull))completionHandler;
